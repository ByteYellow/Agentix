...
Usage:
    python -m agentix.runtime.dump_frame <file>            # auto-detect framed vs raw
    python -m agentix.runtime.dump_frame --framed <file>   # length-prefixed worker frame
    python -m agentix.runtime.dump_frame --raw    <file>   # bare msgpack (HTTP body, SIO arg)

Reads bytes from `<file>` (or stdin if `-`), unpacks via `agentix.runtime.codec`,
...
from agentix.runtime.codec import unpack


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="agentix.runtime.dump_frame", description=__doc__,
