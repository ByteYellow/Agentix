...
below has an in-tree precedent. Host-side helpers (trace sinks, the
three built-in wire patterns, internal spec resolvers, the CLI
dispatcher) are plain Python APIs — they do not participate in
entry-point discovery. See the concept pages —
[Namespace](/namespace) and [Deployment](/deployment) — for what each
axis is.
