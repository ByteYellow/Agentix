...
2. `agentix build` packages the project + every transitive dep into a
   self-contained runtime image — no `FROM`, no `pip install` at run
   time, just a Nix closure under `/nix/store/...`.
3. Deploy a sandbox: the runtime image is overlaid onto a task base
...
- **`runtime_image`** — the bundle from `agentix build`. Generic and
...
async def main() -> None:
    ...
    config = SandboxConfig(
        ...
        # Runtime overlay from `agentix build`.
...
```bash
# Once per distinct runtime_image — stopped container holds its /nix.
