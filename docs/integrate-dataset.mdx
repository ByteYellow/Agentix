...
[tool.setuptools]
package-dir = { "agentix.plugins.datasets.swe" = "src" }
packages = ["agentix.plugins.datasets.swe"]
```

When a rollout bundle depends on `agentix-dataset-swe`, the worker can
import `agentix.plugins.datasets.swe` and call `score`.
